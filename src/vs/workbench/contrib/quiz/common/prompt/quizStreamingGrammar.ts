...
// Aligned with Copilot's prompt/common/streamingGrammar.ts

import { Iterable } from '../../../../../base/common/iterator.js';

export interface IQuizToken<S> {
	state: S;
	transitionTo?: S;
	token: string;
}
...
export class QuizStreamingGrammar<S extends string | number> {
	 ...
	 * 'transitionTo' is set for tokens seen in the grammar when a state transition happens.
	 */
	public readonly tokens: IQuizToken<S>[] = [];
	...
	public append(deltaText: string): Iterable<IQuizToken<S>> {
		let maxLength = 0;
	...
	public flush(): Iterable<IQuizToken<S>> {
		if (this.accumulator) {
